...
R_ADMIN_ADD = re.compile(
    r"^\s*(?:admin(?:\s+add)?|เพิ่ม�?อดมิน)(?:\s+|(?=@)|$)",
    re.IGNORECASE
)
R_ADMIN_DEL   = re.compile(r"^\s*(?:admin\s+del|ลบ�?อดมิน)\b", re.IGNORECASE)
R_ADMIN_LIST  = re.compile(r"^\s*(?:admin\s+list|เช็ค�?อดมิน|รายชื่อ�?อดมิน)\s*$", re.IGNORECASE)
R_CLOSE_TH    = re.compile(r"^(ปิดรอบ|หยุด�?ทง|ปิด)$")
R_YCONFIRM    = re.compile(r"^(?:T/|/Y|Y)\s*$", re.IGNORECASE)
R_CLEAR_PROFIT = re.compile(r"^ล้าง�?ำไร$", re.IGNORECASE)
...
# ====== WEBHOOK IDEMPOTENCY / �?ัน LINE retry ประมวลผลซ้ำ ======
...
def already_processed_message(message_id: str) -> bool:
    """คืน True ถ้า message id นี้เคยถู�?ประมวลผล�?ล้ว"""
    if not message_id:
        return False
    now = time.time()
    with _processed_msg_lock:
        # เ�?็บ cache ให้เล็�? ไม่ให้ RAM บวม
...
PROFIT_RATE = float(os.getenv("PROFIT_RATE", "0.95"))   # ชนะหั�? 5% = จ่ายสุทธิ 1:0.95
MIDDLE_FEE  = float(os.getenv("MIDDLE_FEE",  "0.03"))   # หั�?เมื่อคืนเงิน (�?ลาง/เสมอ�?บบหั�?)
...
_COOLDOWN_LOCK = threading.Lock()  # <<< เพิ่มตัวล็อ�?

def _should_reply_now(scope_key: str) -> bool:
    """
    ป้อง�?ันตอบถี่เ�?ินไป�?บบอะตอมิ�?: เช็ค + อัปเดต ภายใต้ล็อ�?เดียว�?ัน
    scope_key = คีย์สำหรับคูลดาวน์ (เช่น uid:room)
    """
    if REPLY_COOLDOWN_SEC <= 0:
        return True
    now = _now()
    with _COOLDOWN_LOCK:  # <<< ล็อ�?�?ันชน�?ันข้ามเธรด
...
def save_last_settle(payload: dict):
    """เ�?็บสรุปล่าสุดไว้ให้หลังบ้านเรีย�?ดูได้ โดยไม่ต้อง push (ประหยัดโควต้า)"""
    try:
...
def settle_payload_to_text(p: dict) -> str:
    """�?ปลง payload เป็นข้อความสั้น ๆ (Text)
    หมายเหตุ: �?สดง 'ได้เสีย' = payout - stake (มุมมองลู�?ค้า)
             �?ละ '�?ำไรรอบนี้' = มุมมองเจ้ามือ (profit = stake - payout)
    """
    try:
        def _fmt(n):
            try:
...
# ====== ADMIN ACTION GUARD ======
# �?ัน�?อดมิน�?ดคำสั่งซ้อน ทั้งในระดับ thread �?ละ process เดียว�?ัน
# ใช้ไฟล์ state เพื่อ�?ันเคสที่ server มีหลาย worker �?ล้ว memory rooms ไม่ sync �?ัน
        ...
        with open(_ADMIN_ACTION_LOCK_FILE, "a+b") as f:
            ...
            try:
                try:
                    ...
                    # Windows หรือ environment ที่ไม่มี fcntl จะยัง�?ันซ้อนใน process ด้วย thread lock
                    locked = False
                yield
            finally:
                if locked:
...
def claim_round_action(action: str, room_id: str, pair_no, uid: str = None):
    """จอง action ต่อห้อง/รอบ�?บบ atomic
    return (True, None) ถ้าจองสำเร็จ
    return (False, old_info) ถ้ามีคนทำ action นี้ไป�?ล้ว
    """
    ...
    with _admin_action_file_lock():
        ...
        # ล้างข้อมูลเ�?่า �?ันไฟล์โต�?ละ�?ันรอบเ�?่าค้างข้ามวัน
...
def release_round_action(action: str, room_id: str, pair_no):
    """ปลดล็อ�? action เฉพาะ�?รณีตั้งใจ�?ลับมาเปิดรอบเดิม เช่น R/RESUME"""
    if not room_id or not pair_no:
        return
    with _admin_action_file_lock():
        data = _load_admin_action_state()
...
def has_round_action(action: str, room_id: str, pair_no) -> bool:
    """เช็คว่า action ของห้อง/รอบนี้เคยถู�?จองไว้�?ล้วหรือยัง"""
    if not room_id or not pair_no:
        return False
    now = time.time()
    with _admin_action_file_lock():
...
def get_active_round_action(action: str, room_id: str):
    """คืน action ที่ยังค้างอยู่ของห้องนี้ 1 ราย�?าร เช่น rollback ค้างรอบใดอยู่"""
    if not room_id:
        return None
    now = time.time()
    room_id = str(room_id)
    with _admin_action_file_lock():
        ...
        if not candidates:
            return None
        # เอาราย�?ารล่าสุด/ให�?่สุดตามเวลา เพื่อ�?ัน�?รณีข้อมูลเ�?่าหลงเหลือ
...
def save_pending_rollback_snapshot(room_id: str, round_no: int, uid: str, st: dict):
    """เ�?็บสถานะ�?่อนย้อน เพื่อให้คำสั่ง 'ย�?เลิ�?ย้อน <รอบ>' คืน�?ลับได้อย่างปลอดภัย"""
...
def save_users_persist():
    # ไม่เขียนทันที — �?ค่จุด event ให้ worker ไปเขียนเป็น�?้อน
    _save_event.set()
...
BACKOFFICE_GROUP_IDS = {  # �?ลุ่มหลังบ้าน (รับสรุปพร้อม�?ำไรสุทธิ)
...
BANK = {
    "brand": os.getenv("BANK_BRAND", "�?สิ�?รไทย"),
    "accountNo": os.getenv("BANK_ACCOUNT", "115-336-6086"),
    "owner": os.getenv("BANK_OWNER", "�?ิตติพงษ์ ราชวันดี"),
}
PORT = int(os.getenv("PORT", "5000"))

# ====== LINE BOT API TIMEOUT (�?�?้ safe_reply timeout ไป api.line.me) ======
# ค่าเริ่มต้นของ SDK มั�?สั้นเ�?ินไป ทำให้ SSL/read timeout ง่ายตอนเน็ตช้า
LINE_API_TIMEOUT = (
    int(os.getenv("LINE_CONNECT_TIMEOUT", "15")),  # connect timeout (เพิ่มจา�? 10 → 15)
...
PLAY_HELP_TEXT = (
"�?ติ�?า�?ารเล่น\n"
"ต/1000 = �?ทงต่ำ 1000 บาท\n"
"ย/1000 = �?ทงต่ำ 1000 บาท\n"
"ล/1000 = �?ทงสูง 1000 บาท\n"
"ส/1000 = �?ทงสูง 1000 บาท\n"
"สามารถใส่เครื่องหมาย /\n"
"หรือไม่ใส่�?็ได้ \n\n"
"✅ ชนะหั�? 5% (จ่ายสุทธิ 1:0.95)\n"
"⛔ ตส หั�? 3%\n"
"⛔ ตจ หั�? 3%\n"
"⛔ ม = ไม่หั�?\n\n"
...
"👉 �?ทงขั้นต่ำ ยั้ง 30-10,000 ต่อ \n"
"1คน\n"
"🫱�?� �?ทงขั้นต่ำ ไล่ 30-10,000 ต่อ 1คน\n\n"
"📢 เพิ่ม ID ตัวเอง พิมพ์ ADD\n"
"📢 ดูยอดบั�?ชีตัวเอง �?ด C\n"
"📢 ย�?เลิ�?�?าร�?ทง �?ด X\n\n\n"
"***ห้ามเว้นวรรค ***\n"
"�?พิมพ์ยอด�?ารเล่นให้ถู�?ต้องด้วยนะครับ�?\n"
"ตัวอย่าง:\n"
"ล/1000 ส/1000 ล1000 ส1000=ไล่\n"
"ย/1000 ต/1000 ย1000 ต1000=ยั้ง\n"
"(ไม่ต้องเว้นวรรค)\n\n"
"หมายเหตุ // 💥�?รณีออ�?ราคา บั้งไฟ หลังปิด ถือว่า จาวทุ�?รณี\n"
"�?ละสนามราคารูด ทาง�?ลุ่มจะไม่เปิดราคา จาวทุ�?�?รณี 💥"
)
...
def _persist_worker():
    while True:
        _save_event.wait()
        time.sleep(0.35)  # รวมคำสั่งภายใน 350ms �?่อนเขียน
...
# โหลดข้อมูลลู�?ค้า+เครดิตจา�?ดิส�?์ (ถ้ามี)
load_users_persist()


# ====== AUTO DELETE: ลบไฟล์ Backup_round เมื่อครบ 1 วัน �?บบไม่ต้องเช็คทุ�?ชั่วโมง ======
# วิธีทำงาน:
# - ตอนสร้างไฟล์ backup_round_*.json จะตั้ง Timer ให้ลบไฟล์นั้นหลังครบ 24 ชั่วโมงพอดี
# - ตอนเปิดบอท จะส�?�?นไฟล์ backup_round ที่ค้างอยู่ 1 ครั้ง �?ล้วตั้งเวลาลบตามอายุไฟล์ที่เหลือ
# - ไม่มี worker เช็คซ้ำทุ�? 1 ชั่วโมง
...
def _delete_backup_round_file(path: str):
    """ลบไฟล์ backup_round 1 ไฟล์ เมื่อครบเวลา โดยไม่�?ตะไฟล์อื่นใน data"""
    try:
...
def schedule_backup_round_delete(path: str):
    ...
    try:
        ...
        # ถ้าไฟล์เ�?ิน 1 วัน�?ล้ว ให้ลบทันที
        if delay_sec <= 0:
...
def _round_refund(x: float) -> int:
    # เลือ�?ได้: floor = ปัดลง, ceil = ปัดขึ้น
    return floor(x)

def _round_profit(x: float) -> int:
    # �?ำไร�?็ใช้ policy เดียว�?ันเพื่อความคงเส้นคงวา
    return floor(x)


# ====== RESULT DEFINITIONS ======
RESULT_DEFS = {
    # ป�?ติ: �?ั่งชนะจ่าย�?ำไรสุทธิ PROFIT_RATE, �?ั่ง�?พ้เสียเต็ม (�?ต่เรา “ตัดตอนวางบิล�?ล้ว�? จึงไม่ต้องหั�?เพิ่มตอนสรุป)
    "ส":  {"label": "สูงชนะ (จ่าย 1 : %.2f)" % PROFIT_RATE, "winner": "HI"},
    "ต":  {"label": "ต่ำชนะ (จ่าย 1 : %.2f)" % PROFIT_RATE, "winner": "LO"},

    # �?ลาง/จาว/เสมอ-หาย
    "�?":  {"label": "�?ลาง (หั�? %.0f%%)" % (MIDDLE_FEE*100), "special": "MIDDLE_FEE"},
    "จ":  {"label": "จาว (คืนเต็ม ไม่หั�?)", "special": "DRAW_0"},
    "ม":  {"label": "เสมอ-หาย (คืนเต็ม ไม่หั�?)", "special": "DRAW_0"},

    # เคสนโยบายพิเศษ
    "ตจ": {"label": "ต่ำเสมอ (หั�? %.0f%%) / สูงเสียเต็ม" % (MIDDLE_FEE*100), "special": "LOW_DRAWFEE_HIGH_LOSE"},
    "ตส": {"label": "ต่ำเสียเต็ม / สูงเสมอ (หั�? %.0f%%)" % (MIDDLE_FEE*100), "special": "LOW_LOSE_HIGH_DRAWFEE"},
}
...
def start_state():
    return {
        ...
        "escrow": {},     # เงินที่ถู�?หั�?ออ�?ไปทันทีเมื่อรับบิล uid -> amount
    }

# �?�?้ไขในฟัง�?์ชัน start_state()
def start_state():
    return {
        "phase": "NONE",  # NONE | OPEN | PAUSED
        "pairNo": 0,
        ...
        "price": {"camp": None, "HI": (None, None), "LO": (None, None)},
        "escrow": {},
        "score_history": [],  # เ�?็บประวัติผลส�?อบั้งไฟวันนี้
        "settling": False,
...
def can_bet(state, uid, side, amount):
    ...
    if existing:
        ...
        if side != existing["side"]:
            return (False, f"�?� ห้าม�?ทงสวน — คุณมีบิลเดิม: {exist_side_th} {fmt(existing['amount'])}  (พิมพ์ X เพื่อย�?เลิ�?�?่อน)")
        else:
            return (False, f"�?� จำ�?ัด 1 บิล/รอบ — คุณมีบิล {exist_side_th} {fmt(existing['amount'])} อยู่�?ล้ว  (พิมพ์ X เพื่อย�?เลิ�?�?่อน)")
    ...
    if amount > USER_SIDE_CAP[side]:
        ...
        return (False, f"�?ั่ง{side_th} ต่อคนเ�?ิน {fmt(USER_SIDE_CAP[side])}")

    # ✅ เช็คเพดานต่อ�?ั่ง (ย้ายออ�?มาให้อยู่นอ�? if ด้านบน)
    if state["totals"][side] + amount > SIDE_CAP[side]:
        ...
        return (
            False,
            f"�?�รับบิลไม่ได้�?�: �?ั่ง{side_th} เต็ม {fmt(side_cap)} - เหลือรับได้อี�? {fmt(remain_to_cap)}"
        )

    # ✅ เช็คเพดานรวมรอบ + �?จ้งคงเหลือ
    round_total = state["totals"]["HI"] + state["totals"]["LO"]
    if round_total + amount > ROUND_CAP:
        remain_round = max(ROUND_CAP - round_total, 0)
        return (False, f"�?�รับบิลไม่ได้�?�: รอบนี้เต็ม {fmt(ROUND_CAP)} - เหลือรับได้อี�? {fmt(remain_round)}")
...
def flex_open(pair_no, note=None):
    body_contents = [
        {"type": "text", "text": "🎯 เริ่ม�?ทงได้ 🎯", "weight": "bold", "size": "xxl", "align": "center", "color": "#22C55E"},
        {"type": "text", "text": "บอทไม่จับ ไม่ได้เสีย ทุ�?�?รณี •", "size": "md", "align": "center", "color": "#EF4444"},
        {"type": "separator", "margin": "lg", "color": "#4B5563"},
        {"type": "text", "text": f"รอบที่ {pair_no}", "align": "center", "size": "lg", "weight": "bold", "color": "#FFFFFF"},
        {"type": "text", "text": f"รอ�?อดมินออ�?ราคาสั�?ครู่", "align": "center", "size": "lg", "weight": "bold", "color": "#DB0A0A"},
    ]
    if note:
        body_contents += [
            {"type": "separator", "margin": "lg", "color": "#4B5563"},
    ...
    return FlexSendMessage(
        alt_text=f"เริ่ม�?ทงได้ รอบที่ {pair_no}",
        contents={
            "type": "bubble",
            "body": {
                "type": "box",
...
def flex_resume(pair_no: int, camp: str):
    return FlexSendMessage(
        alt_text=f"�?ลับมาเปิดรอบ {pair_no}",
        contents={
            "type": "bubble",
                ...
                "contents": [
                    ...
                    },
                    {
                        ...
                        "contents": [
                            ...
                            {
                                "type": "text",
                                "text": "ฮ่ำมันเข้าไปคั�?ๆ หมานๆนะสมาชิ�?",
                                "size": "sm",
                                "color": "#CBD5E1",
                                "wrap": True
                            },
                            {
                                "type": "text",
                                "text": "ย�?เลิ�?บิลพิมพ์ X • ดูบัตรสมาชิ�?พิมพ์ C",
                                "size": "xs",
                                "color": "#94A3B8",
                                "wrap": True
                            }
    ...
    return FlexSendMessage(
        ...
        contents={
            ...
            "body": {
                ...
                "paddingAll": "14px",
                "contents": [
                    {"type": "text", "text": "🎯 ราคามา�?ล้วว!! 🎯", "weight": "bold", "size": "xl", "align": "center", "color": "#16A34A"},
                    {"type": "text", "text": "บอทไม่จับ ไม่ได้เสีย ทุ�?�?รณี", "size": "sm", "align": "center", "color": "#EF4444"},
                    {"type": "separator", "margin": "md"},
                    {"type": "text", "text": f"🟢🚀ชื่อค่าย :  {camp}", "size": "md", "weight": "bold", "wrap": True},
                    {"type": "text", "text": f"🟢ไล่ราคานี้🟢{hi_txt}🟢", "size": "lg", "weight": "bold"},
                    {"type": "separator", "margin": "md"},
                    {"type": "text", "text": "�?� ราคาบั้งไฟ�?อดมิน�?ำหนดตามความเหมาะสม", "size": "sm", "wrap": True},
                    {"type": "text", "text": "�?� ออ�?ราคาบั้งไฟหลังปิด ถือว่าจาวทุ�?�?รณี", "size": "sm", "wrap": True},
                    {"type": "text", "text": f"👉 �?ทงขั้นต่ำ {MIN_BET} - {fmt(MAX_BET)} บาท/คน/รอบ", "size": "sm"},
                    {"type": "text", "text": f"👉 รวมต่อ�?ั่ง/รอบ: สูง {fmt(SIDE_CAP['HI'])} • ต่ำ {fmt(SIDE_CAP['LO'])}", "size": "sm"},
                    {"type": "text", "text": f"👉 อัตราจ่ายชนะ 1 : {PROFIT_RATE:.2f}", "size": "sm"},
                    {"type": "text", "text": f"👉 ออ�?�?ลางหั�? {int(MIDDLE_FEE*100)}%", "size": "sm"},
                    {"type": "text", "text": "👉 ซุ�?ต�?คาถาน,หาย = จาว", "size": "sm"},
                    {"type": "separator", "margin": "md"},
                    {"type": "text", "text": f"🟢🚀ชื่อค่าย :  {camp}", "size": "md", "weight": "bold", "wrap": True},
                    {"type": "text", "text": f"🔴ยั้งราคานี้🔴{lo_txt}🔴", "size": "lg", "weight": "bold"},
                    {"type": "separator", "margin": "md"},
                    {"type": "text", "text": "📢 ย�?เลิ�?�?าร�?ทง �?ด X", "size": "sm"},
                    {"type": "text", "text": "📢 ดูยอดหน้าบั�?ชีตัวเอง �?ด C", "size": "sm"},
                    {"type": "text", "text": "‼�?�?รณีหน้า�?านราคารูดผิดป�?ติ�?อดมินสามารถ�?จ้งย�?เลิ�?ได้‼", "size": "xs", "wrap": True},
                ]
            }
        }
    )
...
def flex_close_notice(pair_no):
    # �?าร์ด�?จ้งหยุด�?ทง (ปิดรอบ) โทนเดียว�?ับตัวอย่าง
    return FlexSendMessage(
                ...
                "contents": [
                    ...
                    {"type": "box", "layout": "vertical", "backgroundColor": "#111827",
                     "cornerRadius": "12px", "paddingAll": "14px", "contents": [
                         {"type": "text", "text": "หยุด�?ทง", "weight": "bold",
                          "size": "xxl", "align": "center", "color": "#EF4444"},
                         {"type": "text", "text": "บอทไม่จับ ไม่ได้เสีย ทุ�?�?รณี",
                          "size": "md", "align": "center", "color": "#FDE68A"}
                     ]},
                    {"type": "text",
                     "text": "ระบบปิดรับบิล�?ล้ว �?รุณารอสรุปผล/ประ�?าศราคาถัดไป",
...
def flex_pause_notice(pair_no: int, camp: str):
    """�?าร์ด�?จ้ง 'พั�?รอบชั่วคราว' พร้อมชื่อค่าย"""
    if not camp:
        camp = "ไม่ระบุค่าย"
    return FlexSendMessage(
        alt_text=f"พั�?รอบชั่วคราว #{pair_no}",
        contents={
            "type": "bubble",
            ...
            "body": {
                ...
                "contents": [
                    {
                        "type": "text",
                        "text": f"หยุด�?ทงชั่วคราว #{pair_no}",
                        "weight": "bold",
                        "size": "xl",
                        "align": "center",
                        "color": "#1F2937"
                    },
                    {
                        ...
                        "contents": [
                            ...
                            {
                                "type": "text",
                                "text": f"ค่าย {camp} รอ�?อดมินเปิดอี�?รอบ",
                                "size": "sm",
                                "align": "center",
                                "color": "#FDE68A"
                            }
                    ...
                    {
                        "type": "text",
                        "text": "หยุด�?ล้วจะไม่สามารถ�?ทงหรือย�?เลิ�?ได้ รอ�?อดมินเปิดอี�?รอบ",
                        "size": "xs",
                        "align": "center",
                        "wrap": True,
                        "color": "#6B7280"
...
def flex_customer_card(st, user):
    """
    �?าร์ดสมาชิ�?�?บบเรียบง่าย โทนสว่าง เหมือนตัวอย่างในรูป
    �?สดง: รูป • ID • ชื่อ • เครดิตคงเหลือ • ราย�?ารเล่น (ถ้ามี)
    """
    # �?ัน�?รณีเรีย�?�?าร์ด�?่อนผู้ใช้ ADD / ไม่มีข้อมูลใน users
    if not user:
        return TextSendMessage(text="�?รุณาพิมพ์ add เพื่อรับไอดี�?่อน")
    ...
    # �?ถบสถานะ (progress look) ความยาวตามสัดส่วน (ปรับได้)
    # หมายเหตุ: Flex ไม่มี progress จริง ๆ ใช้�?ล่องสองชั้นเลียน�?บบ
    ...
    bet_block = {
        ...
        "contents": [
            ...
            # บรรทัดหั�?ล่วงหน้า + เครดิตคงเหลือ (สไตล์ภาพตัวอย่าง)
            {
                "type": "box", "layout": "horizontal", "contents": [
                    {
                        "type": "text",
                        "text": f"หั�?ล่วงหน้า -{fmt(stake_used)}" if have_bet else "",
                        "size": "xs",
                        "color": "#6B7280",
                        "flex": 7
                    },
    ...
    # ถ้าไม่มีบิล ให้�?สดงบรรทัด “ยังไม่มี�?ารเดิมพัน�?
    if not have_bet:
        bet_block = {
            "type": "text",
            "text": "ยังไม่มี�?ารเดิมพันในรอบนี้",
            "size": "sm",
            "color": "#6B7280",
            "wrap": True
        }
    ...
    return FlexSendMessage(
        ...
        contents={
            ...
            "body": {
                ...
                "layout": "vertical",
                "paddingAll": "12px",
                "backgroundColor": "#F3F4F6",   # เทาอ่อนเหมือน�?ชตตัวอย่าง
                "contents": [
                    {
                        "type": "box",
                        "layout": "vertical",
...
def text_bank():
    return TextSendMessage(
        text=(
            ...
            "💰 ออมสิน\n"
            "💳 วรรณวิไล  ชาเมือง�?ุล\n\n"
            "📌 เพื่อป้อง�?ันมิจฉาชีพ ชื่อผู้�?า�?-ถอน ต้องเป็นชื่อเดียว�?ันเท่านั้น⚠�?\n"
            "📌 �?ด C ดูไอดีตัวเองส่งให้�?อดมินได้เลย\n"
        )
    )


def flex_backoffice_button(url: str, label: str = "เปิดหน้า�?า�?เงิน"):
    """ปุ่ม Flex สำหรับเปิดหน้า�?จ้ง�?า�?/�?า�?เงิน (ลิง�?์ DEPOSIT_URL)

    �?�?้ปั�?หา NameError: flex_backoffice_button ไม่ถู�?ประ�?าศ
    """
    u = (url or '').strip() or DEPOSIT_URL
    # �?ันพิมพ์ลิง�?์�?บบไม่มี scheme
    if not re.match(r'^https?://', u, re.IGNORECASE):
        u = 'https://' + u.lstrip('/')

    return FlexSendMessage(
        alt_text='�?า�?เงิน/�?จ้งโอน',
        contents={
            'type': 'bubble',
            'size': 'mega',
                ...
                'contents': [
                    {
                        ...
                        'contents': [
                            ...
                            {
                                'type': 'text',
                                'text': '�?ดปุ่มด้านล่างเพื่อไปหน้า�?จ้ง�?า�?/�?นบสลิป',
                                'size': 'sm',
                                'align': 'center',
                                'wrap': True,
                                'color': '#6B7280'
...
def flex_result_preview(code: str, pair_no: int):
    ...
    meta = {
        ...
        "�?": {"title": f"�?ลาง (คืนเงิน หั�? {int(MIDDLE_FEE*100)}%)", "accent": "#F59E0B", "icon": "🟡", "desc": "คืนเงิน�?บบหั�?ค่าธรรมเนียม"},
        ...
        "ตจ": {"title": f"ต่ำเสมอ (หั�? {int(MIDDLE_FEE*100)}%) / สูงเสียเต็ม", "accent": "#A855F7", "icon": "🟣", "desc": "ตามนโยบายพิเศษ"},
        "ตส": {"title": f"ต่ำเสียเต็ม / สูงเสมอ (หั�? {int(MIDDLE_FEE*100)}%)", "accent": "#A855F7", "icon": "🟣", "desc": "ตามนโยบายพิเศษ"},
    }
    m = meta.get(code, {"title": "ใส่ผลผิดใส่ใหม่", "accent": "#94A3B8", "icon": "⚪", "desc": "ตรวจสอบรหัสผลอี�?ครั้ง"})
    title = m["title"]
    accent = m["accent"]
    icon = m["icon"]
    desc = m["desc"]

    # สีตัวอั�?ษรผล: โทนเขียวสำหรับคืนเต็ม/จาว/ม, โทนป�?ติ�?รณีอื่น
                                ...
                                "contents": [
                                    ...
                                    },
                                    {
                                        ...
                                        "contents": [
                                            ...
                                            {
                                                "type": "text",
                                                "text": "หา�?ต้อง�?ารเปลี่ยนผล: พิมพ์ s<โค้ดผล> อี�?ครั้ง",
                                                "size": "xs",
                                                "color": "#94A3B8",
                                                "wrap": True
                                            }
...
def flex_settle(pair_no, rows, footer_text,
                ...
                balance_map=None,
                accum=None,
                camp_name=None):  # <--- รับตัว�?ปร camp_name เพิ่ม
    ...
    # --- 1. ส่วนหัวข้อ (Header) ปรับใหม่ให้โชว์ รอบ �?ละ ค่าย ---
    header_contents = [
        # บรรทัดที่ 1: รอบที่ (ตัวให�?่ สีทอง)
        {
            "type": "text",
            "text": f"รอบที่ {pair_no}",
            "weight": "bold",
    ...
    if camp_name:
        ...
        # ถ้าไม่มีชื่อค่าย ให้ขึ้นว่า สรุปผล�?าร�?ทง �?ทน
        header_contents.insert(0, {
             "type": "text", "text": "📊 สรุปผล�?าร�?ทง", 
             "weight": "bold", "align": "center", "size": "md", "color": "#FFFFFF"
        })

    # --- 2. ส่วนราย�?ารผู้เล่น (Body) ---
    header_cols = [
    ...
    # --- 3. ส่วนสรุป�?ำไร (Footer) ---
    if show_profit:
        lines.append({"type": "separator", "margin": "md", "color": "#4B5563"})
        lines.append({"type": "text", "text": f"💰 �?ำไรรอบนี้: {_fmt_signed(profit_value)}",
                      "align": "end", "weight": "bold", "size": "md", "color": "#FACC15"})
        if has_accum:
            lines.append({"type": "text",
                          "text": f"📈 สะสม�?ำไร: {fmt(accum['profit_sum'])} • ขาดทุน: {fmt(accum['loss_sum'])}",
...
def flex_scoreboard(history_list):
    # Mapping ผล: เปลี่ยนจา�?สีพื้นหลัง เป็นสีตัวอั�?ษร (color)
    res_map = {
        "ส":  {"text": "สูง ✅", "color": "#22C55E"},
        "ต":  {"text": "ต่ำ �?�", "color": "#EF4444"},
        "�?":  {"text": "�?ลาง ⛔", "color": "#EAB308"},
    ...
    # ===== 🔥 จุด�?�?้จริง: คัดเหลือผลล่าสุดต่อรอบ =====
    latest_by_round = {}
    for h in history_list or []:
        r = h.get("round")
        if r is None:
            continue
        latest_by_round[r] = h   # ตัวหลังทับตัว�?่อน (ผลล่าสุด)

    # เรียงตามรอบ �?ล้วเอา 10 รอบล่าสุด
...
def flex_call_pages(user_rows, title="ตารางเครดิตลู�?ค้า", per_page=30):
    pages = []
    total = len(user_rows)
    ...
    # ========== HEAD TABLE: ขนาดเล็�?ลง 100% ทำงานได้ ==========
    def table_head():
        return {
            "type": "box",
            "layout": "horizontal",
    ...
    for i in range(num_pages):
        ...
        if chunk:
            for idx, r in enumerate(chunk):
                ...
                rows.append({
                    "type": "box",
                    "layout": "horizontal",
                    "paddingAll": "4px",   # ลด padding �?ต่ไม่ถึงขั้นพัง
        ...
        summary = {
            ...
            "contents": [
                {"type": "box", "layout": "horizontal", "contents": [
                    {"type": "text", "text": "ลู�?ค้าในหน้านี้", "flex": 6, "size": "xs", "color": "#6B7280"},
...
def rules_text() -> str:
    return (
        "🎉🎊�?ติ�?า🎊🎉\n"
        f"✨�?ทงขั้นต่ำ{MIN_BET}-{fmt(MAX_BET)}บาท/คน/รอบ\n"
        "\n"
        "�?��?ารจ่าย🎖�?\n"
        ...
        f"🟢ออ�?�?ลางเจ๊า หั�? {int(MIDDLE_FEE*100)}%\n"
        "\n"
        "- จำ�?ัด 1 บิล/รอบ �?ละห้าม�?ทงสวน (ต้องย�?เลิ�?บิลเดิม�?่อน)\n"
        "- พิมพ์ x เพื่อย�?เลิ�?บิล / พิมพ์ C เพื่อดูบัตรสมาชิ�?\n"
    )
...
def settle_by_code(st, code):
    """
    โมเดลเครดิต:
    - ตอนรับบิล: หั�?เครดิต = amount �?ละเ�?็บ st['escrow'][uid] += amount
    - ตอนสรุป:
        * ชนะ: คืนต้นทุน + �?ำไรสุทธิ (amount + amount*PROFIT_RATE)
        * �?พ้: ไม่คืนอะไร (ต้นทุนถู�?หั�?ไป�?ล้ว)
        * คืนเงินหั�? fee: คืน amount*(1 - fee)
        * คืนเต็ม: คืน amount
    ฟัง�?์ชันนี้คืน rows: [{'uid','name','stake','payout'}...], footer_text
    """
    acc = {}
    ...
    # �?ลางคืนเงิน หั�? MIDDLE_FEE
    if d.get("special") == "MIDDLE_FEE":
        for b in st["bet_index"].values():
            refund = _round_refund(b["amount"] * (1 - MIDDLE_FEE))
            add(b["uid"], b["name"], b["amount"], refund)
        return list(acc.values()), f"ผล: �?ลาง (คืนเงิน หั�? {int(MIDDLE_FEE*100)}%)"

    # ต่ำเสมอ (หั�? fee) / สูงเสียเต็ม
    if d.get("special") == "LOW_DRAWFEE_HIGH_LOSE":
        ...
        return list(acc.values()), f"ผล: ต่ำเสมอ (หั�? {int(MIDDLE_FEE*100)}%) / สูงเสียเต็ม"

    # ต่ำเสียเต็ม / สูงเสมอ (หั�? fee)
    if d.get("special") == "LOW_LOSE_HIGH_DRAWFEE":
        for b in st["bet_index"].values():
            if b["side"] == "HI":
                ...
                add(b["uid"], b["name"], b["amount"], 0)
        return list(acc.values()), f"ผล: ต่ำเสียเต็ม / สูงเสมอ (หั�? {int(MIDDLE_FEE*100)}%)"
...
def safe_reply(event, messages):
    """Reply message �?บบไม่ทำให้บอทล่ม + retry เมื่อ timeout"""
...
def safe_push(to_id, messages, label: str = "", return_reason: bool = False):
    """Push message �?บบไม่ทำให้บอทล่ม + retry เมื่อ timeout
...
def _lockdown_and_alert(gid, uid):
    """ป้อง�?ัน kick สำคั�?: ล็อ�?ดาวน์ + �?จ้งเตือน (no-op safety)."""
    LOCKDOWN_UNTIL[gid] = _now() + LOCKDOWN_SECONDS_DEFAULT
    try:
        name = _member_name(gid, uid)
        safe_push(gid, TextSendMessage(f"⚠�? �?ลุ่มล็อ�?ดาวน์ชั่วคราว {LOCKDOWN_SECONDS_DEFAULT} วินาที เพราะสมาชิ�?สำคั�?ออ�?: {name}"))
    except Exception:
        pass
...
<title>คัดลอ�?เลขบั�?ชี • �?สิ�?รไทย</title>
<style>
  :root {{
...
<body>
  <main class="card" role="main" aria-labelledby="title">
    <div class="bank">
      <img src="{logo_url}" alt="ธนาคาร�?สิ�?รไทย" class="bank-logo" loading="lazy" decoding="async"
           referrerpolicy="no-referrer"
           onerror="this.remove();document.getElementById('kbank-fallback').style.display='block';">
      <div>
        <div id="title" class="bank-title">ธนาคาร�?สิ�?รไทย</div>
        <div class="subtitle" aria-hidden="true">ธนาคาร�?สิ�?รไทย</div>
    ...
    <div class="acct-wrap" tabindex="0" aria-live="polite" aria-atomic="true">
      <div class="label">เลขบั�?ชี</div>
      <div id="acct" class="acct" data-raw="{acct}"></div>
    </div>

    <p class="help">�?ตะ “คัดลอ�?เลขบั�?ชี�? �?ล้วสลับไปที่�?อปธนาคารเพื่อวาง�?ละโอนเงิน</p>

    <div class="btns">
      <button id="copyBtn" class="primary" aria-label="คัดลอ�?เลขบั�?ชี">คัดลอ�?เลขบั�?ชี</button>
      <button id="closeBtn" class="secondary" aria-label="ปิดหน้านี้">ปิดหน้านี้</button>
    </div>

    <div id="status" class="status warn">�?ำลังเตรียมคัดลอ�?ให้อัตโนมัติ…</div>
    <p class="help" style="margin-top:14px">เคล็ดลับ: ถ้าคัดลอ�?ไม่ติด ให้�?ดปุ่ม “คัดลอ�?เลขบั�?ชี�? อี�?ครั้ง</p>
...
(function() {{
  ...
  async function doCopy() {{
    ...
    try {{
      ...
      statusEl.textContent = 'คัดลอ�?�?ล้ว ✓ นำไปวางใน�?อปธนาคารได้เลย';
      statusEl.className = 'status ok';
    }} catch(e) {{
      statusEl.textContent = 'คัดลอ�?ไม่สำเร็จ �?รุณา�?ดปุ่ม “คัดลอ�?เลขบั�?ชี�? อี�?ครั้ง';
  ...
  function robustClose() {{
    ...
    setTimeout(() => {{
      ...
      try {{
        ...
        statusEl.textContent = 'ปิด�?ท็บนี้ได้เลย';
        ...
        contents={
            ...
            "body": {
                ...
                "contents": [
                    ...
                    {"type": "text", "text": "พิมพ์ C เพื่อดูบัตรสมาชิ�?", "size": "xs", "align": "center", "color": "#9CA3AF"}
                ]
            }
        }
    )
...
def flex_summary(st, event=None):
    ...
    if not bets:
        ...
        for i, b in enumerate(bets):
            bg_color = "#1E293B"   # ใช้สีเดียวทุ�?�?ถว
            name = b["name"]
            if b["side"] == "HI":
                side_display = "✅ สูง"
                side_color = "#22C55E"
    ...
    # ===== Flex Message =====
    return FlexSendMessage(
        alt_text=f"📋 สรุป�?าร�?ทง คู่ที่ {st['pairNo']}",
        contents={
            "type": "bubble",
            ...
            "body": {
                ...
                "contents": [
                    # ส่วนหัว
                    {
                        ...
                        "contents": [{
                            "type": "text",
                            "text": f"📊 สรุป�?าร�?ทง รอบ {st['pairNo']} ({len(bets)})",
                            "weight": "bold",
                            "align": "center",
                            "size": "lg",
                            "color": "#FFFFFF"
...
def _dedupe_admin_ids(ids):
    """คืน list �?อดมิน�?บบตัดซ้ำ �?ต่คงลำดับเดิม"""
    seen = set()
    out = []
    for x in ids or []:
        x = (x or "").strip()
...
def save_admins_persist():
    """บันทึ�?รายชื่อ�?อดมินลงไฟล์ data/admins.json เพื่อให้ restart �?ล้วไม่หาย"""
    try:
...
def load_admins_persist():
    """โหลดรายชื่อ�?อดมินจา�? data/admins.json มารวม�?ับ ADMIN_IDS ใน .env"""
    try:
...
def add_admin(uid):
    """เพิ่ม�?อดมิน�?ละบันทึ�?ถาวร คืน True ถ้าเพิ่มใหม่ / False ถ้ามีอยู่�?ล้ว"""
    with _admin_ids_lock:
        if uid in ADMIN_IDS:
            return False
...
def remove_admin(uid):
    """ลบ�?อดมิน�?ละบันทึ�?ถาวร คืน True ถ้าลบสำเร็จ / False ถ้าไม่พบ"""
    with _admin_ids_lock:
...
# โหลด�?อดมินที่เคยเพิ่มผ่านคำสั่งใน LINE ให้�?ลับมาหลัง restart/deploy
load_admins_persist()
...
def register_customer_by_uid(src, target_uid):
    """สมัครสมาชิ�?ให้ target_uid �?ละคืน (user_dict, created_new: bool)"""
...
def process_credit_command(text, uid):
    if not is_admin(uid):
        return "คำสั่งนี้ใช้ได้เฉพาะ�?อดมิน"
    m = re.match(r"^@([^\s]+)\s*/\s*(\d+)$", text)
    if not m: return "รูป�?บบคำสั่งไม่ถู�?ต้อง (ตัวอย่าง: @สมชาย/500)"
...
def on_message(event: MessageEvent):
    ...
    # [FIXED] �?ำหนด text ที่นี่ครั้งเดียว
    text = (event.message.text or "").strip()

    # �?ัน LINE retry / webhook ซ้ำ: message id เดิมต้องไม่ถู�?ประมวลผลซ้ำ
    ...
    if gid:
        ...
        if _locked_group(gid) and not is_admin(uid):
            if not _notice_throttled(uid):
                safe_reply(event, TextSendMessage("ระบบ: �?ลุ่ม�?ำลังล็อ�?ดาวน์ชั่วคราว ติดต่อ�?อดมินเพื่อปลดล็อ�?"))
            return
    ...
    if MUTED_UNTIL.get(uid, 0) > _now():
        if not _notice_throttled(uid):
            safe_reply(event, TextSendMessage("ระบบ: คุณถู�?จำ�?ัด�?ารส่งข้อความชั่วคราว (anti-spam)"))
        return
    ...
    if not rl.allow(f"uid:{uid}:burst", RL_UID_BURST_LIMIT, RL_UID_BURST_PERIOD) or \
       not rl.allow(f"uid:{uid}:day", RL_UID_DAILY_LIMIT, RL_UID_DAILY_PERIOD):
        ...
        if STRIKES[uid] >= ABUSE_STRIKE_TO_MUTE:
            ...
            STRIKES[uid] = 0
            if not _notice_throttled(uid):
                safe_reply(event, TextSendMessage(f"ระบบ: มิวท์ {MUTE_SECONDS_DEFAULT} วินาที เนื่องจา�?ข้อความถี่ผิดป�?ติ"))
        else:
            if not _notice_throttled(uid):
                safe_reply(event, TextSendMessage("ระบบ: ข้อความถี่เ�?ิน�?ำหนด ช่วยเว้นช่วงหน่อยนะ"))
        return
    ...
    # [FIXED] รวบรวมตรร�?ะทั้งหมดที่เ�?ี่ยวข้อง�?ับ Room State (st) ไว้ใน Lock เดียว
    with with_rooms_lock():
        ...
        if not is_admin(uid):
            whitelist = {"add", "c", "�?ต", "บช", "x", "xx", "x*", "ถอน", "วิธีเล่น", "วิธี�?ารเล่น", "เล่น"}
            ...
            if head not in whitelist:
                if not _should_reply_now(scope_key):
                    # เงียบ: ไม่ตอบ�?ละไม่ประมวลผลคำสั่ง เพื่อ�?ันรัวจริง ๆ
                    return
        # --- คำสั่งล้าง�?ำไรทั้งหมด (เฉพาะ Admin) ---
        if R_CLEAR_PROFIT.match(text):
            if uid not in ADMIN_IDS:
                return  # ไม่ใช่�?อดมินไม่ต้องตอบโต้
            ...
            reply_msg = (
                "✅ รีเซ็ตข้อมูล�?ำไรทั้งหมดเรียบร้อย�?ล้ว\n"
                f"🕒 เวลา: {now}\n"
        ...
        # ===== วิธีเล่น / เล่นยังไง / เล่นไง / วิธี�?ารเล่น / เล่น�?บบใด =====
        t = text.strip()
        ...
        # �?รณีผู้ใช้พิมพ์�?บบมีเว้นวรรค เช่น "เล่น ยังไง"
        if t2.startswith("เล่น") and ("ยังไง" in t2 or "ไง" in t2 or "�?บบใด" in t2):
            safe_reply(event, TextSendMessage(PLAY_HELP_TEXT))
            return
        ...
        # ใช้ได้ทั้ง: admin add @ชื่อ / admin @ชื่อ / เพิ่ม�?อดมิน @ชื่อไลน์
        if R_ADMIN_ADD.match(text):
            ...
            # เผื่อ�?รณีพิมพ์ UID ตรง ๆ เช่น: เพิ่ม�?อดมิน Uxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
            if not target_uid:
            ...
            if not target_uid:
                safe_reply(event, TextSendMessage(
                    "�?� �?รุณา�?ท็�?ชื่อผู้ใช้ที่ต้อง�?ารเพิ่ม\nตัวอย่าง: เพิ่ม�?อดมิน @ชื่อไลน์"
                ))
                return
            ...
            if add_admin(target_uid):
                ...
                safe_reply(event, TextSendMessage(
                    f"ℹ�? ผู้ใช้นี้เป็น�?อดมินอยู่�?ล้ว\n👤 {target_name}"
                ))
            return

        if R_ADMIN_DEL.match(text):
            if not is_admin(uid):
                safe_reply(event, TextSendMessage("คำสั่งนี้ใช้ได้เฉพาะ�?อดมิน")); return
            target_uid = first_mentioned_uid(event)
            if not target_uid:
                m = re.search(r"\b([Uu][0-9a-f]{32})\b", text)
                if m: target_uid = m.group(1)
            if not target_uid:
                safe_reply(event, TextSendMessage("โปรด�?ท็�?ผู้ใช้ หรือระบุ userId ที่ขึ้นต้นด้วย U...")); return
            pin = _admin_auth_pin(text)
            if ADMIN_PIN and not compare_digest(pin, ADMIN_PIN):
                safe_reply(event, TextSendMessage("PIN ไม่ถู�?ต้อง")); return
            if remove_admin(target_uid):
                safe_reply(event, TextSendMessage("ลบ�?อดมินสำเร็จ ✓"))
            else:
                safe_reply(event, TextSendMessage("ไม่พบไอดีนี้ในรายชื่อ�?อดมิน"))
            return
        
        # ===== Admin: list (เช็ค�?อดมิน / admin list) =====
        if R_ADMIN_LIST.match(text):
            ...
            if not current_admins:
                safe_reply(event, TextSendMessage("ℹ�? ยังไม่มี�?อดมินในระบบ")); return
            lines = ["👑 รายชื่อ�?อดมินทั้งหมด\n"]
        ...
        if re.match(r"^gid\b", text, re.IGNORECASE):
            if not gid:
                safe_reply(event, TextSendMessage("ใช้คำสั่งนี้ได้เฉพาะใน�?ลุ่ม/ห้อง"))
                return
            safe_reply(event, TextSendMessage(f"GID ของ�?ลุ่มนี้: {gid}"))
            return


        # ==== ดูตารางราย�?ารเดิมพัน (cm — เฉพาะ�?ลุ่มหลังบ้าน) ====
        if text.lower() == "cm":
            if not gid or not is_backoffice_group_id(gid):
                return
            snapshot = [(rk, stx.copy()) for rk, stx in rooms.items()]  # shallow �?็พอ
            ...
            rows.extend([
                {"type":"box","layout":"horizontal","spacing":"sm","contents":[
                    ...
                    {"type":"text","text":"จำนวนเดิมพัน/�?ี่บาท","flex":5,"size":"xs","align":"center","weight":"bold","wrap":True},
        ...
        if m_cmd:
            ...
            if not target_uid:
                ...
                if m_uid:
                    ...
                    if m_at:
                        safe_reply(event, TextSendMessage("โปรด�?ท็�?ผู้ใช้จา�? UI ของ LINE (ชื่อเป็นลิง�?์สีน้ำเงิน)"))
                        return
            if not target_uid:
                safe_reply(event, TextSendMessage("รูป�?บบ: mute @ผู้ใช้ [วินาที] / unmute @ผู้ใช้ / ban @ผู้ใช้ / unban @ผู้ใช้"))
                return
            if cmd == "mute":
                if sec is None:
                    m_sec = re.search(r"\b(\d+)\b$", args) if args else None
                    sec = int(m_sec.group(1)) if m_sec else MUTE_SECONDS_DEFAULT
                MUTED_UNTIL[target_uid] = _now() + max(1, sec); safe_reply(event, TextSendMessage(f"มิวท์ {sec} วินาที�?ล้ว")); return
            if cmd == "unmute":
                MUTED_UNTIL.pop(target_uid, None); safe_reply(event, TextSendMessage("ปลดมิวท์�?ล้ว")); return
            if cmd == "ban":
                BANNED_UIDS.add(target_uid); safe_reply(event, TextSendMessage("�?บนผู้ใช้�?ล้ว")); return
            if cmd == "unban":
                BANNED_UIDS.discard(target_uid); safe_reply(event, TextSendMessage("ปลด�?บน�?ล้ว")); return

        # ===== Admin/User: เช็ค UID =====
        if re.match(r"^uid\b", text, re.IGNORECASE):
            target_uid = first_mentioned_uid(event)
            # ถ้าไม่�?ท็�?ใคร: โชว์ UID ของตัวเอง (ไม่ต้องเป็น�?อดมิน)
            if not target_uid or target_uid == uid:
                ...
                return
            # ถ้าจะดู UID คนอื่น ต้องเป็น�?อดมิน
            if not is_admin(uid):
                safe_reply(event, TextSendMessage("เฉพาะ�?อดมินเท่านั้นที่ดู UID คนอื่นได้")); 
                return
            name, _ = get_profile_display(event.source, target_uid)
            safe_reply(event, TextSendMessage(f"UID ของ {name}: {target_uid}"))
            return

                # ===== Backoffice (FREE): ดูสรุป/�?ำไรล่าสุด =====
        # ใช้ใน�?ลุ่มหลังบ้านเท่านั้น (ไม่ต้อง push ลดโควต้า)
        if gid and gid in BACKOFFICE_GROUP_IDS and re.match(r"^(?:�?ำไรล่าสุด|ยอด|lastprofit|last)\b", text, re.IGNORECASE):
            if not is_admin(uid):
                safe_reply(event, TextSendMessage("คำสั่งนี้ใช้ได้เฉพาะ�?อดมิน")); 
                return
            p = load_last_settle()
            if not p:
        ...
        if R_GETID.match(text):
            ...
            if group_id:
                ...
                msg = "�?� คำสั่งนี้ใช้ได้เฉพาะใน�?ลุ่มหรือห้องเท่านั้น"
        ...
        if re.match(r"^(clear|reset)\b", text, re.IGNORECASE):
            ...
            with with_users_lock(): # [FIXED] ใช้�?ค่ with_users_lock() เพราะ with_rooms_lock() คลุมอยู่�?ล้ว
                if re.search(r"\ball\b", text, re.IGNORECASE):
                    # เคลียร์ทั้งระบบ — คืน escrow ทุ�?ห้อง
                    ...
                    # 👉 เพิ่มบรรทัดนี้: รีเซ็ต�?ำไรสะสม
                    METRICS["profit_sum"] = 0
                    METRICS["loss_sum"] = 0
                    clear_round_action_guard()

                    # ลบไฟล์ backup ทั้งหมด
                    for _bfname in os.listdir(DATA_DIR):
                        _bfpath = os.path.join(DATA_DIR, _bfname)
                        if _is_backup_round_file(_bfname):
                            try: os.remove(_bfpath)
                            except Exception: pass
                    # รีเซ็ตเครดิตลูกค้าทั้งหมดเป็น 0
                    for u in users.values():
                        u["credit"] = 0
                        u["escrow"] = 0
                    msg = "Clear All OK - reset credits + deleted backups"
                else:
        ...
        # ==== เติม/ลบทุน+เครดิต �?บบ $+ <cid> <amt> / $- <cid> <amt> ====
        ...
        if m_del:
            ...
            with with_users_lock():
                ...
                if has_active_bet(target_uid):
                    safe_reply(event, TextSendMessage("�?� ลบไม่ได้: ลู�?ค้ามีบิลค้างอยู่"))
                    return

                users.pop(target_uid)
                save_users_persist()

            safe_reply(event, TextSendMessage(f"✅ ลบลู�?ค้า ID {cid} สำเร็จ"))
            return        



        # ===== เติมเครดิตรูป�?บบ @ชื่อ/จำนวน =====
        if text.startswith("@") and "/" in text:
            msg = process_credit_command(text, uid)
            safe_reply(event, TextSendMessage(msg)); return

        # ==== ลู�?ค้า: สมัคร/�?าร์ด/บั�?ชี/�?ต ====
        if re.match(r"^add\b", text, re.IGNORECASE):
            target_uid = first_mentioned_uid(event)

            # �?อดมินสมัครสมาชิ�?�?ทนลู�?ค้าด้วย�?าร�?ท็�?ชื่อ
            if target_uid and target_uid != uid:
                if not is_admin(uid):
                    safe_reply(event, TextSendMessage("เฉพาะ�?อดมินเท่านั้นที่สมัครสมาชิ�?�?ทนลู�?ค้าได้"))
                    return
                target_user, created = register_customer_by_uid(event.source, target_uid)
                if created:
                    safe_reply(event, TextSendMessage(
                        f"✅ สมัครสมาชิ�?ให้ {target_user['name']} สำเร็จ\n🎫 ID: {target_user['cid']}\nพิมพ์ C @ชื่อไลน์ เพื่อดูบัตรสมาชิ�?"
                    ))
                else:
                    safe_reply(event, TextSendMessage(
                        f"ℹ�? {target_user['name']} มี ID อยู่�?ล้ว: {target_user['cid']}"
                    ))
                return

            # สมัครสมาชิ�?ด้วยตัวเอง
            if text.lower() == "add":
                target_user, created = register_customer_by_uid(event.source, uid)
                if not created:
                    safe_reply(event, TextSendMessage(f"คุณมี ID �?ล้ว: {target_user['cid']}"))
                    return
                safe_reply(event, flex_register_success(target_user["cid"])); return

        # ลู�?ค้าพิมพ์ "ถอน" ที่ไหน�?็ได้ในข้อความ → �?สดง�?าร์ด C
        if "ถอน" in text:
            u = users.get(uid)
            if not u:
                safe_reply(event, TextSendMessage("พิมพ์ add เพื่อรับไอดี�?่อน"))
                return
        ...
        if re.match(r"^c\b", text, re.IGNORECASE):
            ...
            # �?อดมินดูบัตร/ID ของลู�?ค้าที่ถู�?�?ท็�?
            if target_uid and target_uid != uid:
                if not is_admin(uid):
                    safe_reply(event, TextSendMessage("เฉพาะ�?อดมินเท่านั้นที่ดูข้อมูลลู�?ค้าคนอื่นได้"))
                    return
                u = users.get(target_uid)
                if not u:
                    safe_reply(event, TextSendMessage("ลู�?ค้ายังไม่ได้สมัครสมาชิ�?\nให้�?อดมินพิมพ์ add @ชื่อไลน์ �?่อน"))
                    return
                safe_reply(event, flex_customer_card(st, u)); return

            # ลู�?ค้าดูบัตรของตัวเอง
            if text.lower() == "c":
                u = users.get(uid)
                if not u:
                    safe_reply(event, TextSendMessage("พิมพ์ add เพื่อรับไอดี�?่อน"))
        ...
        if text.strip().lower() in ("บช", "บั�?ชี", "เลขบั�?ชี"):
        ...
        # ==== ประ�?าศราคา�?บบ "ส่งข้อความอย่างเดียว" (ไม่เปิดรอบ) ====
        # ==== ประ�?าศราคา�?บบ "ส่งข้อความอย่างเดียว" (ไม่เปิดรอบ) ====
        m_announce = R_ANN.match(text)

        if m_announce and not re.match(r"^\s*o\b", text, re.IGNORECASE):
            if not is_admin(uid):
                safe_reply(event, TextSendMessage("คำสั่งประ�?าศราคานี้ใช้ได้เฉพาะ�?อดมิน"))
                return
        ...
        if re.match(r"^\s*o\b", text, re.IGNORECASE):
            ...
            if st["phase"] != "NONE":
                phase_th = "เปิดอยู่" if st["phase"] == "OPEN" else "พั�?รอบอยู่"
                safe_reply(
                    event,
                    TextSendMessage(
                        f"�?� เปิดรอบใหม่ไม่ได้: ยังมีรอบค้างอยู่ ({phase_th})\n"
                        f"ขั้นตอนที่ถู�?ต้อง: �?ด E เพื่อพั�? → พิมพ์ s<รหัสผล> → /y เพื่อยืนยันผล\n"
                        f"เมื่อสรุปรอบเสร็จ�?ล้ว จึงค่อยเปิดรอบใหม่ได้"
                    )
                ); return
        ...
        if t == "E":
            ...
            if not claimed:
                ...
                safe_reply(event, TextSendMessage(
                    f"⚠�? รอบ {st['pairNo']} ปิดรับบิลไป�?ล้ว ไม่ต้องปิดซ้ำ"
                ))
                return
            ...
            # ส่ง 2 ข้อความ: (1) �?าร์ดพั�?รอบ (2) สรุปบิล
            try:
                safe_reply(event, [
                ...
                # �?ันต�? ถ้ามีปั�?หา Flex จะยังตอบเป็นข้อความได้
                safe_reply(event, TextSendMessage(f"พั�?รอบชั่วคราว #{st['pairNo']} — ค่าย {camp}"))
            return
        ...
        if t in ("R", "RESUME"):
            if not is_admin(uid):
                ...
                return
            if st["phase"] != "PAUSED":
                safe_reply(event, TextSendMessage("ไม่มีรอบที่พั�?อยู่")); return
            ...
            # ==== ปิดรอบ (ข้อความไทย: ปิดรอบ / หยุด�?ทง / ปิด) ====
        if R_CLOSE_TH.match(text.strip()):
            if not is_admin(uid):
                safe_reply(event, TextSendMessage("คำสั่งนี้ใช้ได้เฉพาะ�?อดมิน")); return

            # สำคั�?: ปิดได้เฉพาะตอน OPEN เท่านั้น
            # ถ้า PAUSED อยู่�?ล้ว ห้ามส่ง�?าร์ดปิด/สรุปซ้ำ
            if st["phase"] != "OPEN":
                if st["phase"] == "PAUSED":
                    safe_reply(event, TextSendMessage(
                        f"⚠�? รอบ {st['pairNo']} ปิดรับบิลไป�?ล้ว ไม่ต้องปิดซ้ำ\n"
                        f"ขั้นตอนถัดไป: พิมพ์ s<รหัสผล> �?ล้ว�?ดยืนยัน /y"
                    )); return
                if st["phase"] == "SETTLING" or st.get("settling"):
                    safe_reply(event, TextSendMessage("�?� ระบบ�?ำลังสรุปผลอยู่ �?รุณารอสั�?ครู่")); return
            ...
            # เปลี่ยนสถานะเป็นพั�?รอบทันที �?่อนส่งข้อความ เพื่อ�?ัน�?อดมินอี�?คน�?ดซ้อน
            st["phase"] = "PAUSED"
            st["last_closed_pairNo"] = st["pairNo"]

            # ส่ง�?าร์ด�?จ้งหยุด�?ทง + สรุปบิลรอบนี้
            safe_reply(event, [
        ...
        if sm:
            if not is_admin(uid):
                safe_reply(event, TextSendMessage("คำสั่งนี้ใช้ได้เฉพาะ�?อดมิน")); return
            if st["phase"] != "PAUSED":
                safe_reply(event, TextSendMessage("�?� ต้อง�?ด E (พั�?รอบ) �?่อนจึงจะตั้งผลได้")); return
        ...
        if R_YCONFIRM.match(text.strip()):
            ...
            # �?ัน�?อดมิน�?ด /y ซ้อน หรือ LINE retry ระหว่าง�?ำลังคำนวณเครดิต
            if st.get("settling") or st["phase"] == "SETTLING":
                safe_reply(event, TextSendMessage("�?� ระบบ�?ำลังสรุปผลอยู่ คำสั่งยืนยันซ้ำถู�?ย�?เลิ�?")); return

            if st["phase"] != "PAUSED":
                if st.get("last_settled_pairNo") == st.get("pairNo"):
                    safe_reply(event, TextSendMessage(f"⚠�? รอบ {st['pairNo']} สรุปผลไป�?ล้ว ไม่สามารถยืนยันซ้ำได้")); return
                safe_reply(event, TextSendMessage("�?� ต้อง�?ด E (พั�?รอบ) �?่อนจึงจะสามารถสรุปผลได้")); return

            if not st.get("pendingCode"):
                safe_reply(event, TextSendMessage(
                    f"�?� ยังไม่ได้ตั้งผลรอบ {st['pairNo']}\n"
                    f"�?รุณาพิมพ์ s<รหัสผล> �?่อน �?ล้วค่อย�?ดยืนยัน /y"
                )); return

            code = normalize_result_code(st["pendingCode"])
            if code not in RESULT_DEFS:
                safe_reply(event, TextSendMessage("�?� ไม่สามารถยืนยันผลได้: โค้ดผลไม่ถู�?ต้อง")); return
            ...
            if not claimed:
                ...
                st["settling"] = False
                safe_reply(event, TextSendMessage(
                    f"⚠�? รอบ {st['pairNo']} �?ำลังถู�?สรุปผล หรือสรุปผลไป�?ล้ว คำสั่งยืนยันซ้ำถู�?ย�?เลิ�?"
                )); return

            # ล็อ�?สถานะทันที �?่อนเริ่ม backup/คำนวณ/คืนเครดิต
            st["phase"] = "SETTLING"
            st["settling"] = True


            # ================= [เริ่มส่วนที่เพิ่ม] =================
            # 1. บันทึ�?ประวัติลง State
            ...
            # === [เพิ่มใหม่] บันทึ�? snapshot เครดิต + สถานะห้อง �?่อนสรุปผล ===
            try:
                backup_path = os.path.join(DATA_DIR, f"backup_round_{st['pairNo']}.json")
                with with_users_lock(): # [FIXED] ใช้�?ค่ with_users_lock()
                    snapshot = {
                        "round": st["pairNo"],
                        "users": users,
                        "room_state": st.copy(),   # ✅ st อยู่ใน with_rooms_lock() อยู่�?ล้ว
                        "metrics": METRICS.copy(), 
                    }
                    _atomic_write_json(backup_path, snapshot)

                # ตั้งเวลาลบ backup_round ไฟล์นี้เมื่อครบ 1 วัน โดยไม่ต้องเช็คทุ�?ชั่วโมง
                schedule_backup_round_delete(backup_path)

                app.logger.info(f"[Backup] บันทึ�?เครดิต�?่อนสรุปผล รอบ {st['pairNo']} สำเร็จ")
            except Exception as e:
                app.logger.warning(f"[Backup] ไม่สามารถบันทึ�? snapshot รอบ {st['pairNo']}: {e}")
            # === [จบส่วนเพิ่มใหม่] ===


            # คำนวณยอด �?ละคืนเครดิตให้ผู้เล่น
            ...
            # ถ้ารอบนี้เคยถู�?ย้อน�?ล้ว �?ละตอนนี้ออ�?ผลใหม่สำเร็จ�?ล้ว
            # ให้ปลดล็อ�? rollback/pending snapshot เพื่อให้ย้อนผลรอบนี้ได้อี�?ครั้งเฉพาะหลังออ�?ผลใหม่เท่านั้น
            ...
            # [FREE] เ�?็บสรุปล่าสุดไว้ให้หลังบ้านเรีย�?ดูได้ (ไม่ต้อง push = ประหยัดโควต้า)
            try:
                rows_payload = []
                for r in rows:
                    rr = dict(r)
            ...
            # 2. ส่งให้ห้องลู�?ค้า (�?สดงชื่อค่ายด้วย)
            safe_reply(event, [
            ...
            # 3) หลังบ้าน�?บบฟรี (�?นะนำ): ให้พิมพ์ "�?ำไรล่าสุด" ใน�?ลุ่มหลังบ้านเพื่อดึงผลล่าสุด
            ...
            if os.getenv("BACKOFFICE_PUSH_ENABLED", "0") == "1":
                ...
                if p:
                    ...
                    # ส่ง�?ค่ 1 �?ลุ่ม�?ร�? เพื่อลดโควต้า (ปรับได้ถ้าต้อง�?าร)
                    bo_targets = BACKOFFICE_GROUP_IDS[:1]
                    for gid_to in bo_targets:
                        safe_push(gid_to, msg, label="backoffice_text")

            return
        # ==== ย�?เลิ�?ย้อนผล (Cancel Rollback) ====
        m_cancel_rollback = re.match(r"^(?:ย�?เลิ�?ย้อน|cancel\s+rollback|cancelrollback)\s*(\d+)$", text.strip(), re.IGNORECASE)
        if m_cancel_rollback:
            ...
            if not active_rb:
                safe_reply(event, TextSendMessage(
                    f"ℹ�? ตอนนี้ไม่มีราย�?ารย้อนผลค้างอยู่ จึงไม่ต้องย�?เลิ�?ย้อน {round_no}"
                )); return

            active_round = int(active_rb.get("pair_no") or 0)
            if active_round != round_no:
                safe_reply(event, TextSendMessage(
                    f"⚠�? ตอนนี้ราย�?ารย้อนผลที่ค้างอยู่คือรอบ {active_round}\n"
                    f"ถ้าจะย�?เลิ�? ให้พิมพ์: ย�?เลิ�?ย้อน {active_round}\n"
                    f"ยังไม่สามารถย�?เลิ�?ย้อน {round_no} ได้"
                )); return

            snap = load_pending_rollback_snapshot(key)
            if not snap or int(snap.get("round_no") or 0) != round_no:
                safe_reply(event, TextSendMessage(
                    f"�?� ไม่พบสถานะ�?่อนย้อนของรอบ {round_no}\n"
                    f"เพื่อป้อง�?ันเครดิตเพี้ยน �?รุณาตั้งผลรอบ {round_no} ใหม่ด้วย s<รหัสผล> �?ล้ว�?ดยืนยัน /y ให้เสร็จ�?่อน"
                )); return

            try:
                # คืนสถานะ�?ลับไป�?่อนคำสั่งย้อนผล เหมือนย�?เลิ�?�?ารย้อนจริง ๆ
                with with_users_lock():
                    users.clear()
                ...
                # �?ลับสถานะเป็นออ�?ผล�?ล้ว จึงต้องล็อ�? settle รอบนี้ไว้เหมือนเดิม
                if not has_round_action("settle", key, round_no):
                    claim_round_action("settle", key, round_no, uid)
                clear_pending_rollback_snapshot(key)

                safe_reply(event, TextSendMessage(
                    f"✅ ย�?เลิ�?ย้อนผลรอบ {round_no} สำเร็จ�?ล้ว\n"
                    f"ระบบคืนสถานะ�?ลับไป�?่อนย้อนผลเรียบร้อย\n"
                    f"ตอนนี้สามารถย้อนผลรอบอื่นได้�?ล้ว"
                )); return
            except Exception:
                app.logger.exception("cancel rollback failed")
                safe_reply(event, TextSendMessage(
                    f"�?� ย�?เลิ�?ย้อนผลรอบ {round_no} ไม่สำเร็จ\n"
                    f"เพื่อความปลอดภัย �?รุณาตั้งผลรอบ {round_no} ใหม่ด้วย s<รหัสผล> �?ล้ว /y ให้จบ�?่อน"
                )); return
        ...
        if m_rollback:
            ...
            # ห้ามย้อนตอนระบบ�?ำลังสรุปผล เพื่อ�?ันเครดิต/ไฟล์ snapshot ชน�?ัน
            if st.get("settling") or st.get("phase") == "SETTLING":
                safe_reply(event, TextSendMessage(
                    f"�?� ระบบ�?ำลังสรุปผลรอบ {st.get('pairNo')} อยู่ ห้ามย้อนผลระหว่างนี้"
                )); return

            # ล็อ�? global ต่อห้อง: ถ้าย้อนรอบใดค้างอยู่ ห้ามย้อนรอบอื่นจน�?ว่าจะออ�?ผลใหม่หรือย�?เลิ�?ย้อน
            active_rb = get_active_round_action("rollback", key)
            if active_rb:
                active_round = int(active_rb.get("pair_no") or 0)
                if active_round == round_no:
                    safe_reply(event, TextSendMessage(
                        f"⚠�? รอบ {round_no} ถู�?ย้อนผลไป�?ล้ว\n"
                        f"ต้องตั้งผลรอบ {round_no} ใหม่ด้วย s<รหัสผล> �?ล้ว�?ดยืนยัน /y ให้เสร็จ�?่อน\n"
                        f"หรือพิมพ์: ย�?เลิ�?ย้อน {round_no}\n"
                        f"ห้ามพิมพ์ย้อนผลซ้ำ"
                    )); return
                else:
                    safe_reply(event, TextSendMessage(
                        f"⚠�? ตอนนี้มีราย�?ารย้อนผลรอบ {active_round} ค้างอยู่\n"
                        f"ต้องจัด�?ารรอบ {active_round} ให้เสร็จ�?่อน จึงจะย้อนผลรอบ {round_no} ได้\n"
                        f"ทางเลือ�? 1: ตั้งผลรอบ {active_round} ใหม่ด้วย s<รหัสผล> �?ล้ว�?ดยืนยัน /y\n"
                        f"ทางเลือ�? 2: พิมพ์ ย�?เลิ�?ย้อน {active_round} �?ล้วค่อยย้อนผลรอบ {round_no}"
                    )); return

            # ถ้ายังไม่เคยสรุปผลรอบนี้จริง ห้ามย้อน เพราะไม่มีผลให้ย้อน
            if not has_round_action("settle", key, round_no):
                safe_reply(event, TextSendMessage(
                    f"�?� รอบ {round_no} ยังไม่ได้ออ�?ผล หรือยังไม่มีราย�?ารสรุปผลที่ยืนยัน�?ล้ว\n"
                    f"ย้อนผลได้เฉพาะรอบที่ออ�?ผล�?ล้วเท่านั้น"
                )); return
            ...
            if not os.path.exists(backup_file):
                safe_reply(event, TextSendMessage(
                    ...
                    f"ไม่สามารถย้อนผลได้ เพื่อป้อง�?ันเครดิตเพี้ยน"
                )); return

            # เ�?็บ snapshot ปัจจุบันไว้�?่อน rollback เพื่อให้คำสั่ง 'ย�?เลิ�?ย้อน <รอบ>' คืน�?ลับได้
            try:
                save_pending_rollback_snapshot(key, round_no, uid, st)
            except Exception:
                safe_reply(event, TextSendMessage(
                    f"�?� เตรียมข้อมูลย�?เลิ�?ย้อนรอบ {round_no} ไม่สำเร็จ\n"
                    f"ระบบยังไม่ย้อนผล เพื่อป้อง�?ันเครดิตเพี้ยน"
                )); return
            ...
            if not claimed:
                clear_pending_rollback_snapshot(key)
                safe_reply(event, TextSendMessage(
                    f"⚠�? รอบ {round_no} ถู�?ย้อนผลไป�?ล้ว หรือ�?ำลังถู�?ย้อนผลอยู่\n"
                    f"ต้องออ�?ผลรอบ {round_no} ใหม่ให้เสร็จ�?่อน ห้ามย้อนซ้ำ"
                )); return
            ...
            try:
                ...
                st["phase"] = "PAUSED"
                st["settling"] = False
                st["pendingCode"] = None  # บังคับให้�?อดมินตั้งผลใหม่ ไม่ใช้ผลเ�?่าโดยไม่ตั้งใจ

                # ✅ คืนค่า�?ำไรสะสม (METRICS)
                if "metrics" in data:
                    METRICS.clear()
                    METRICS.update(data["metrics"])

                # ปลดล็อ�? settle เดิม เพราะรอบนี้ถู�?ย้อน�?ลับมา�?ล้ว ต้องอนุ�?าตให้ออ�?ผลใหม่ได้
                release_round_action("settle", key, round_no)

                safe_reply(event, TextSendMessage(
                    f"✅ ย้อนเครดิต�?ละข้อมูลรอบ {round_no} สำเร็จ�?ล้ว\n"
                    f"ขั้นตอนต่อไป: พิมพ์ s<รหัสผล> �?ล้ว�?ดยืนยัน /y\n"
                    f"ถ้าไม่ต้อง�?ารย้อน�?ล้ว ให้พิมพ์: ย�?เลิ�?ย้อน {round_no}\n"
                    f"ระหว่างนี้ห้ามย้อนรอบอื่นจน�?ว่าจะจัด�?ารรอบ {round_no} ให้เสร็จ"
                )); return
            except Exception:
                # ถ้าย้อนล้มเหลว ให้ปลดล็อ�? rollback เพื่อให้�?�?้ไข/ลองใหม่ได้ ไม่ให้ค้างถาวร
                release_round_action("rollback", key, round_no)
                clear_pending_rollback_snapshot(key)
                app.logger.exception("rollback failed")
                safe_reply(event, TextSendMessage(
                    f"�?� ย้อนผลรอบ {round_no} ไม่สำเร็จ ระบบย�?เลิ�?คำสั่งนี้�?ล้ว"
                )); return
        ...
        if text.strip().lower() == "call":
            ...
            if not table:
                safe_reply(event, TextSendMessage("ยังไม่มีลู�?ค้าที่มีเครดิต"))
                return
        ...
        # เช็คว่าเป็นคำสั่งย�?เลิ�?หรือไม่
        if text.strip().lower() in ("xx", "x*") or m_cancel_by_cid or text.strip().upper() == "X":
            if st["phase"] == "NONE":
                safe_reply(event, TextSendMessage("ย�?เลิ�?ไม่ได้: รอบนี้สรุปจบ�?ล้ว")); return
            ...
            if text.strip().lower() in ("xx", "x*"):
                ...
                with with_users_lock():
                    ...
                    st["totals"] = {"HI": 0, "LO": 0}
                    save_users_persist()
                extra = " (�?ำลังพั�?รอบ)" if st["phase"] == "PAUSED" else ""
                safe_reply(event, TextSendMessage(f"ย�?เลิ�?บิลทั้งหมดสำเร็จ{extra} ({n} บิล)")); return

            # �?อดมินย�?เลิ�?ตาม ID ลู�?ค้า
            if m_cancel_by_cid:
                if not is_admin(uid):
                    safe_reply(event, TextSendMessage("คำสั่งนี้ใช้ได้เฉพาะ�?อดมิน")); return
                ...
                with with_users_lock():
                    ...
                    save_users_persist()
                extra = " (�?ำลังพั�?รอบ)" if st["phase"] == "PAUSED" else ""
                safe_reply(event, TextSendMessage(f"ย�?เลิ�?บิลของ ID {cid} สำเร็จ{extra} ({'สูง' if bet['side']=='HI' else 'ต่ำ'} {fmt(bet['amount'])})")); return

            # ลู�?ค้าย�?เลิ�?บิลตัวเอง
            if text.strip().upper() == "X":
                if st["phase"] == "PAUSED":
                    safe_reply(event, TextSendMessage("�?ำลังพั�?รอบ: ลู�?ค้าไม่สามารถย�?เลิ�?ได้ โปรดให้�?อดมินดำเนิน�?าร")); return
                bet = st["bet_index"].pop(uid, None)
                if not bet:
                    safe_reply(event, TextSendMessage("คุณยังไม่มี�?ารเดิมพันในรอบนี้")); return
                ...
                safe_reply(event, TextSendMessage(f"คุณ {line_name} �?�ย�?เลิ�?�?ารเดิมพันเดิมสำเร็จ�?� ({'สูง' if bet['side']=='HI' else 'ต่ำ'} {fmt(bet['amount'])})")); return

        # ==== FAST PATH: ส่วนนี้ต้องอยู่นอ�? if ด้านบน ====
        bet = parse_bet(text)
        if bet:
            # �?อดมินเล่นได้ (ปิด�?ารเช็ค is_admin ไว้�?ล้ว)
            # if is_admin(uid):
            #     return
            
            with with_users_lock(): # [FIXED] ใช้�?ค่ with_users_lock()
                if uid not in users:
                    safe_reply(event, TextSendMessage("�?รุณาพิมพ์ add เพื่อรับไอดี�?่อนวางบิล")); return
    ...
    # ----- ที่เหลือค่อยไปเช็คคำสั่ง�?อดมิน/ยูทิลต่างๆ เหมือนเดิม -----
...
def on_image(event: MessageEvent):
    ...
    # ต้องอยู่ใน�?ลุ่ม/ห้องเท่านั้น
    if not in_group_or_room(event.source):
        return

    # ตรวจ allow/deny/lockdown ของ�?ลุ่ม
    if gid:
        if gid in BANNED_GROUPS or gid in DENY_GROUP_IDS:
            return
        if not is_allowed_group(gid):
    ...
    # rate limit �?บบเดียว�?ับข้อความตัวหนังสือ
    if not rl.allow(f"room:{key}", RL_ROOM_BURST_LIMIT, RL_ROOM_BURST_PERIOD):
        return
    if not rl.allow(f"uid:{uid}:burst", RL_UID_BURST_LIMIT, RL_UID_BURST_PERIOD) or \
       not rl.allow(f"uid:{uid}:day", RL_UID_DAILY_LIMIT, RL_UID_DAILY_PERIOD):
        ...
        if STRIKES[uid] >= ABUSE_STRIKE_TO_MUTE:
            ...
            STRIKES[uid] = 0
            if not _notice_throttled(uid):
                safe_reply(event, TextSendMessage(f"ระบบ: มิวท์ {MUTE_SECONDS_DEFAULT} วินาที เนื่องจา�?�?ิจ�?รรมถี่ผิดป�?ติ"))
        else:
            if not _notice_throttled(uid):
                safe_reply(event, TextSendMessage("ระบบ: �?ิจ�?รรมถี่เ�?ิน�?ำหนด ช่วยเว้นช่วงหน่อยนะ"))
        return
    ...
    # ต้องมีข้อมูลผู้ใช้�?่อน (พิมพ์ add มา�?่อน)
    with with_users_lock():
        u = users.get(uid)

    if not u:
        safe_reply(event, TextSendMessage("�?รุณาพิมพ์ add เพื่อรับไอดี�?่อน"))
        return

    # ตอบ�?าร์ด C ของผู้ที่ส่งรูป
    try:
        safe_reply(event, flex_customer_card(st, u))
    except Exception:
        # �?ันต�? ถ้า Flex error ให้ตอบข้อความธรรมดา โดยไม่ให้พังซ้ำถ้า u หาย
...
# ตั้ง token ได้ด้วย env: BACKOFFICE_VIEW_TOKEN (ถ้าไม่ตั้ง จะเปิดได้เฉพาะในวง�?ลน/เครื่องตัวเองตามไฟร์วอลล์)
